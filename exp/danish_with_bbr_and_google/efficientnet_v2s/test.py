from util.util import EarlyStopper, MetricLogger, compute_scores, init_write, compute_scores_seg, create_confusion_matrix, output_label
...
parser = argparse.ArgumentParser(description="Test script for model loading")
...
file_path =f'logs/test/test_{formatted_time}'
fig_path1 = 'logs/confusion_metrics_herlev.png'
fig_path2 = 'logs/confusion_metrics'
...
if efficient:
    if s:
        ...
        model.load_state_dict(torch.load("models/efficientnet_v2_l.pth", map_location=device))

    model.avgpool = nn.AdaptiveAvgPool2d((1,1)) #occhio che era 2,2
    ...
    "models/segformer_b0_classification_local")
...
if herlev:
    testpath = '/leonardo_work/PHD_cozzani/seg_solarbackup/dataset/danish_dataset1/herlev_test/test/'
    ...
    if efficient or vgg:
        ...
        create_confusion_matrix(model, test_loader, device, file_path, fig_path1, seg= segformer)
    else: 
...
if bologna:                                                                                                                              
    for test in ['merged', 'casalecchio', 'mengoli', 'suburbano2']:
        testpath = os.path.join('/leonardo_work/PHD_cozzani/seg_solarbackup/dataset/dataset_bolo_class/', test)
        testset= CustomDataset(testpath, classes=['positive', 'negative'], transform = transform)
        test_loader = DataLoader(testset, batch_size = 32, num_workers =8)
        if efficient or vgg:
            accuracy, precision, recall, val_loss = compute_scores(model, test_loader, device)
            create_confusion_matrix(model, test_loader, device, file_path, fig_path2+'_'+ test + '.png', seg= segformer)
        else:
            accuracy, precision, recall, val_loss = compute_scores_seg(model, test_loader, device)
            create_confusion_matrix(model, test_loader, device, file_path, fig_path2+'_'+ test + '.png', seg= segformer)
