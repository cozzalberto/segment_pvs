...
def train():
    ...
    torch.cuda.set_device(local_rank)
    model = models.vgg16()
    model.load_state_dict(torch.load("models/vgg16_weights.pth", map_location=f"cuda:{local_rank}"))
    ...
    if rank == 0:
        torch.save(ddp_model.module.state_dict(), f"logs/nofreezeVGG_{formatted_time}.pth")
