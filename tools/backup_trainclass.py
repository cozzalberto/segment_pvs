...
trainset_filtered = CustomDataset(root_dir='dataset/danish_dataset2/solardk_dataset_neurips_v2/gentofte_trainval/train', classes=['positive', 'negative'], transform=training_transform)
...
valset = CustomDataset(root_dir='dataset/danish_dataset2/solardk_dataset_neurips_v2/gentofte_trainval/val', classes=['positive', 'negative'], transform=transform)
...
def train():
    ...
    optimizer = optim.Adam([{'params':backbone_params,'lr':3e-4},{'params': classifier_params, 'lr':0.001}], weight_decay=5e-4) # LR più basso, aggiungi weight_decay
    scheduler = StepLR(optimizer,step_size=11,gamma=0.1)
    #Definisci l'ottimizzatore per i parametri che richiedono il gradiente (solo il classificatore inizialmente)   
    num_epochs_phase1 = 15 #17 # Addestra solo il classificatore per 5 epoche (puoi sperimentare)
    if rank == 0:
    ...
    for epoch in range(num_epochs_phase1):
        ...
        if rank == 0:
            ...
            with open(f"logs/log_{formatted_time}.txt",'a') as f:
                f.write(f"[Fase 1 - Epoch {epoch+1}/{num_epochs_phase1}] Train Loss: {train_loss_avg:.4f} Val Loss: {val_loss_avg:.4f} Val Acc: {val_acc:.4f} Prec: {val_precision_avg:.4f} Recall: {val_recall_avg:.4f}\n")
    ...
    num_epochs_phase2 = 25 # Numero di epoche per il fine-tuning (potrebbe essere di più)
    fine_tune_lr = 1e-5 # Learning rate molto più basso per il fine-tuning
    ...
    #scheduler = OneCycleLR(optimizer, max_lr=fine_tune_lr * 10, epochs=num_epochs_phase2, steps_per_epoch=len(dataloader))
    #scheduler = StepLR(optimizer,step_size= 3,gamma=0.1)
    stopper_phase2 = EarlyStopper(patience = 15)
    if rank == 0:
    ...
    if rank == 0:
        torch.save(ddp_model.module.state_dict(), "nofreeze_16batchlungo.pth")
        with open(f"logs/logtorchvision_{formatted_time}.txt",'a') as f:
        ...
        plt.savefig('logs/metrics_aug.png')
    cleanup()
