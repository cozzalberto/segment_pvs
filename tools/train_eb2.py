...
file_path = f'logs/log_eb2_{formatted_time}.txt'
...
transform = A.Compose([
    A.Resize(height=260, width=260, interpolation=1, p=1.0),
...
training_transform = A.Compose([
    A.Resize(height=260, width=260, interpolation=1, p=1),
...
wd2 = 5e-4
num_steplr = 25
scheduler1 ='reduce'   #SCHEDULER1 È QUELLO DEL BACKBONE, SCHEDULER_DIFF È QUELLO DEL CLASSIFIER. STESSO CRITERIO PER I LEARNING RATE. ANCHE SE NUM_UNFROZEN1 È 0 È COSÌ!!!!
...
valset = CustomDataset(root_dir='/leonardo_work/PHD_cozzani/seg_solarbackup/dataset/danish_dataset1/gentofte_trainval/val', classes=['positive', 'negative'], transform = transform)
...
def train():
    ...
    model.load_state_dict(torch.load("models/efficientnet_b2_imagenet.pth", map_location=f"cuda:{local_rank}"))
    ...
    if num_unfrozen1>0:
        ...
        optim_classifier = torch.optim.AdamW(classifier_params, lr=learning_diff, weight_decay= wd1)
        ...
        if scheduler1.lower()=='steplr':
            ...
            scheduler_backbone = ReduceLROnPlateau(optim_backbone, factor = 0.5,  patience =5)
        else:
        ...
        if scheduler_diff.lower()=='steplr':
            ...
            scheduler_classifier = ReduceLROnPlateau(optim_classifier, factor = 0.5, patience = 5)
        else:
            raise ValueError("learning rate scheduler not recognized")
    else:
        optim_classifier = torch.optim.AdamW(classifier_params, lr=learning_diff, weight_decay= wd1)
