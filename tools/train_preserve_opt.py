...
transform = A.Compose([
    A.Resize(height=426, width=426, interpolation=1, p=1.0),
...
training_transform = A.Compose([
    A.Resize(height=426, width=426, interpolation=1, p=1),
    A.HorizontalFlip(p=0.3),                  # Step 2: Basic geometric
    A.OneOf([
        A.ToGray(p=1.0),
...
def train():
    ...
    if num_unfrozen1>0:
        ...
        if scheduler1.lower()=='steplr':
            ...
            scheduler_backbone = ReduceLROnPlateau(optim_backbone, factor = 0.6,  patience =3)
    ...
    for epoch in range(num_epochs_phase1):
        ...
        if num_unfrozen1>0:
            if scheduler1.lower()!='reduce':
                scheduler_backbone.step()
            else:
                scheduler_backbone.step(val_loss_avg)
        if scheduler_diff.lower()!= 'reduce':
    ...
    print(sum(p.numel() for p in ddp_model.module.parameters() if p.requires_grad))
    new_params  =list(filter(lambda p: p.requires_grad, ddp_model.module.features[-num_unfrozen2:-num_unfrozen1].parameters()))
    # Re-inizializza l'ottimizzatore con il nuovo learning rate e i parametri aggiornati
    optim_backbone.add_param_group({
    'params': new_params,
    'lr': fine_tune_lr,
    'weight_decay': wd1
    }) 
    ...
    if num_epochs_phase2>0:
        if scheduler2.lower()=='steplr':
            ...
            scheduler_class = StepLR(optim_classifier, step_size = num_steplr, gamma = 0.1)
        elif scheduler2.lower()=='cosine':
            scheduler_backbone = CosineAnnealingLR(optim_backbone, T_max = num_epochs_phase2)
            scheduler_class = CosineAnnealingLR(optim_classifier, T_max = num_epochs_phase2)

        elif scheduler2.lower()=='onecycle':
            scheduler_backbone = OneCycleLR(optim_backbone, max_lr= fine_tune_lr *gamma_onecycle , epochs=num_epochs_phase2, steps_per_epoch=len(dataloader))
            scheduler_class = OneCycleLR(optim_classifier, max_lr=fine_tune_lr *gamma_onecycle , epochs=num_epochs_phase2, steps_per_epoch=len(dataloader))

        elif scheduler2.lower()=='reduce':
            scheduler_backbone = ReduceLROnPlateau(optim_backbone, "min", factor = 0.5, patience = 3)
        else:
            raise ValueError("learning rate scheduler not recognized")
        #scheduler = StepLR(optimizer,step_size= 3,gamma=0.1)
    stopper_phase2 = EarlyStopper(patience = 20)
    if rank == 0:
        print("\n--- Fase 2: Fine-tuning dell'intera rete (o sezioni) ---")

    for epoch in range(num_epochs_phase2):
        sampler.set_epoch(epoch +num_epochs_phase1)
        ddp_model.train() # Imposta il modello in modalità training
        running_loss = 0.0
        total_samples = 0
        stop_flag = torch.tensor(False, device = local_rank)
        for inputs, labels in dataloader:
            ...
            running_loss += loss.item() * inputs.size(0)
            total_samples += inputs.size(0)
            if num_unfrozen2>0 and scheduler2.lower() == 'onecycle':
                scheduler_backbone.step()
        ...
        train_loss_avg = logger.get_global_metric(train_loss)
        if num_unfrozen2>0 and scheduler2.lower() != 'onecycle':
            if scheduler2.lower()!='reduce':
                scheduler_backbone.step()
                scheduler_class.step()
            else:
                scheduler_backbone.step(val_loss_avg)
                scheduler_class.step(val_loss_avg)
        ...
        if rank == 0:
            ...
            recall_history.append(val_recall_avg)
            loss_val_history.append(val_loss_avg)
            print(f"[Fase 2 - Epoch {epoch+1}/{num_epochs_phase2}] Train Loss: {train_loss_avg:.4f} Val Loss: {val_loss_avg:.4f} Val Acc: {val_acc:.4f} Prec: {val_precision_avg:.4f} Recall: {val_recall_avg:.4f} Current LR: {optim_backbone.param_groups[0]['lr']:.6f}")
            with open(f"logs/log_{formatted_time}.txt",'a') as f:
                f.write(f"[Fase 2 - Epoch {epoch+1}/{num_epochs_phase2}] Train Loss: {train_loss_avg:.4f} Val Loss: {val_loss_avg:.4f} Val Acc: {val_acc:.4f} Prec: {val_precision_avg:.4f} Recall: {val_recall_avg:.4f} Current LRs: {optim_backbone.param_groups[0]['lr']:.6f} class: {scheduler_class.get_last_lr()[0]}\n")
